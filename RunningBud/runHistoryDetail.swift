...
class runHistoryDetail: UIViewController, MKMapViewDelegate {
    ...
    var runID: Int?
        ...
        private func loadRunHistory() {
            ...
            // 更新界面显示内容
            runPace.text = "\(runHistory.runSpace) min/mile"
            runCal.text = "\(runHistory.runCal) kcal"
            runDis.text = "\(runHistory.runDis) mile"
        ...
        private func setupMap(with coordinates: [[Double]]) {
            ...
            // 计算并打印距离
            calculateAndPrintDistances(for: mapPoints)
        }
        
        // 计算并打印点对点距离以及总距离
        private func calculateAndPrintDistances(for points: [CLLocationCoordinate2D]) {
            ...
            for i in 0..<points.count - 1 {
                ...
                // 打印每段距离
                print("Distance from point \(i) to point \(i + 1): \(distance) meters")
            }
